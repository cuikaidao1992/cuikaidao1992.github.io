...
//只看我的回答
$('#onlyMyOwnQas, #isPrivateChat').bind('click', function (e) {
...
// 优选网络
$('#btn-network').bind('click', function (e) {

    if ($(".video-mask").is(":hidden")) {

        var l = JSON.parse($.DW.getPlayLine());
        var netlist = '';
...
// 选择网络
$('#btn-netsubmit').bind('click', function (e) {
    var network = $('input[name="network"]:checked').val();
    $('#network').fadeOut(200);

    $.DW.changeLine(network);

    e.preventDefault();
});

$('#btn-netcannel').bind('click', function (e) {
    $('#network').fadeOut(200);
